...
(
	...
	NgayTraSan DATETIME DEFAULT GETDATE(),
	MaSan INT NOT NULL,
	MaKH INT NOT NULL,
...
BEGIN
	...
	DECLARE @MaSan INT,  @MaKH INT, @TongPhut INT, @GiaThue DECIMAL, @ThanhTien DECIMAL, @THOIGIANBD DATETIME,  @THOIGIANKT DATETIME, @MaLich INT
	SELECT @MaSan = MaSan,
       @MaKH = MaKH,
	...
	Select @GiaThue = (Select GiaThue
					  FROM LOAISAN, SANBONG, inserted i
					  WHERE LOAISAN.MaLoai = SANBONG.MaLoai
					  AND i.MaSan = SanBong.MaSan)

	UPDATE LICHDATSAN
    SET THANHTIEN = (@GiaThue/60) * @TongPhut
    FROM LICHDATSAN, inserted i
	where LICHDATSAN.MaLich = i.MaLich

	SET @ThanhTien = (Select ThanhTien
					  from LICHDATSAN
					  where LICHDATSAN.MaLich = @MaLich
					  )

	INSERT INTO HOADON (NgayDatSan, NgayTraSan, MaKH, MaSan, TongPhut, DonGia, TongTien)
	VALUES (@THOIGIANBD, @THOIGIANKT, @MaKH,@MaSan, @TongPhut, @GiaThue, @ThanhTien)
...
BEGIN
	...
	DECLARE @MaSan INT,  @MaKH INT, @TongPhut INT, @GiaThue DECIMAL, @ThanhTien DECIMAL, @THOIGIANBD DATETIME,  @THOIGIANKT DATETIME, @MaLich INT, @TienDichVu DECIMAL
	SELECT @MaSan = MaSan,
       @MaKH = MaKH,
       @THOIGIANBD = THOIGIANBD,
       @THOIGIANKT = THOIGIANKT,
	...
	UPDATE HOADON
	SET TONGTIEN = @ThanhTien + (TongTien - d.ThanhTien), NgayDatSan = @ThoiGianBD, NgayTraSan = @ThoiGianKT, MaKH = @MaKH, TongPhut = @TongPhut, DonGia = @GiaThue, MaSan = @MaSan
	FROM deleted d
    WHERE HOADON.NgayDatSan = d.ThoiGianBD
	AND HOADON.NgayTraSan = d.ThoiGianKT
	AND HOADON.MaKH = d.MaKH
	AND HOADON.MaSan = d.MaSan
	AND HOADON.TrangThai = N'Chưa thanh toán'
...
GO
CREATE TRIGGER TR_KHACHHANG_CapNhatDiaChiSDTKhiUpdate
ON KHACHHANG
AFTER UPDATE
AS
BEGIN
...
--Bảo
INSERT INTO ACCOUNT
VALUES ('admin1', 'e10adc3949ba59abbe56e057f20f883e', N'Đạt', 1),
('admin2', '0192023a7bbd73250516f069df18b500', N'Tài', 1),
('admin3', 'a66abb5684c45962d887564f08346e8d', N'Tú', 1),
('nhanvien1', 'f5fce1662295e09e104dc99a27978510', N'Bảo', 0),
('nhanvien2', 'c1aaf44c2a50277efd113a040ffe6f77', N'Trí', 0)
-- Đạt
INSERT INTO LOAISAN
...
AS
BEGIN
	SELECT * FROM ACCOUNT WHERE UserName LIKE '%' + @str + '%' OR Password LIKE '%' + @str + '%' OR DisplayName LIKE '%' + @str + '%' OR Role LIKE '%' + @str + '%'
END
GO
...
as 
begin
	SELECT MAHD, NgayDatSan, NgayTraSan, NgayTao, MAKH, HOADON.MASAN, TongPhut, DonGia, TONGTIEN, TrangThai
	FROM HOADON, SANBONG
